...
async def create_order(order: OrderCreate):
    ...
    with db.engine.connect() as conn:
        # Адаптивный запрос для пользователя
        ...
        # Вставляем элементы заказа если таблица существует
...
async def get_order(order_id: str):
    ...
    with db.engine.connect() as conn:
        ...
        # Получаем элементы заказа если таблица существует
        items = []
        total_price = 0.0
        ...
        if _table_exists('order_items'):
            ...
            # Упрощаем если нет некоторых таблиц
            if not _table_exists('bar_items'):
                items_query = """
                    SELECT 
                        'Блюдо' as type,
...
async def get_new_orders_for_kitchen():
    ...
    with db.engine.connect() as conn:
        # Адаптивный запрос в зависимости от структуры
